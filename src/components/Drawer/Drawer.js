...
function Drawer({ onClose, onRemove, items = [], opened }) {
  ...
  const onClickOrder = async () => {
    try {
      setIsLoading(true);
      const { data } = await axios.post('https://61f908cc783c1d0017c448d4.mockapi.io/orders', {
        items: cartItems,
      });
      setOrderId(data.id);
      ...
      for (let i = 0; i < cartItems.length; i++) {
        ...
        await axios.delete('https://61f908cc783c1d0017c448d4.mockapi.io/cart/' + item.id);
        await delay(1000);
      }
    } catch (error) {
      ...
      <div className={styles.drawer}>
        ...
        {items.length > 0 ? (
          ...
          <Info
            ...
            image={isOrderComplete ? completeOrder : cartEmpty}
          />
        )}
      </div>
    </div>
